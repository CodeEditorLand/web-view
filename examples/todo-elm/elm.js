((scope) => {
	...
	var _List_sortBy = F2((f, xs) =>
		_List_fromArray(
			_List_toArray(xs).sort((a, b) => _Utils_cmp(f(a), f(b))),
		),
	);
	...
	var _Debug_log_UNUSED = F2((tag, value) => value);

	var _Debug_log = F2((tag, value) => {
	...
	function _Json_toElmArray(array) {
		return A2($elm$core$Array$initialize, array.length, (i) => array[i]);
	}
...
*/
	...
	var _Platform_incomingPortMap = F2(
		(tagger, finalTagger) => (value) => tagger(finalTagger(value)),
	);
	...
	var _VirtualDom_init = F4(
		(virtualNode, flagDecoder, debugMetadata, args) => {
	...
	//*/
			...
			node.parentNode.replaceChild(
				_VirtualDom_render(virtualNode, () => {}),
				node,
			);
	...
	function _VirtualDom_makeCallback(eventNode, initialHandler) {
		function callback(event) {
			...
			var value = result.a;
			var message = tag ? (tag < 3 ? value.a : value.message) : value;
			var stopPropagation =
	...
	function _Browser_application(impl) {
		...
		return _Browser_document({
			...
			init: (flags) => A3(impl.init, flags, _Browser_getUrl(), key),
			view: impl.view,
			update: impl.update,
	...
	function _Browser_getViewportOf(id) {
		return _Browser_withNode(id, (node) => ({
			scene: {
	...
	var $elm$core$Set$toList = (_v0) => {
		var dict = _v0.a;
		return $elm$core$Dict$keys(dict);
	};
	var $elm$core$Elm$JsArray$foldr = _JsArray_foldr;
	var $elm$core$Array$foldr = F3((func, baseCase, _v0) => {
		var tree = _v0.c;
		var tail = _v0.d;
	...
	var $elm$core$Result$Err = (a) => ({ $: "Err", a: a });
	...
	var $elm$core$Result$Ok = (a) => ({ $: "Ok", a: a });
	var $elm$json$Json$Decode$OneOf = (a) => ({ $: "OneOf", a: a });
	var $elm$core$Basics$False = { $: "False" };
	var $elm$core$Basics$add = _Basics_add;
	var $elm$core$Maybe$Just = (a) => ({ $: "Just", a: a });
	...
	var $elm$core$Array$Leaf = (a) => ({ $: "Leaf", a: a });
	...
	var $elm$core$Array$SubTree = (a) => ({ $: "SubTree", a: a });
	...
	var $elm$browser$Browser$External = (a) => ({ $: "External", a: a });
	var $elm$browser$Browser$Internal = (a) => ({ $: "Internal", a: a });
	var $elm$core$Basics$identity = (x) => x;
	var $elm$browser$Browser$Dom$NotFound = (a) => ({ $: "NotFound", a: a });
	...
	var $elm$core$Task$Perform = (a) => ({ $: "Perform", a: a });
	...
	var $elm$core$Task$map = F2((func, taskA) =>
		A2(
			...
			(a) => $elm$core$Task$succeed(func(a)),
			taskA,
		),
	);
	...
	var $elm$core$Task$spawnCmd = F2((router, _v0) => {
		var task = _v0.a;
	...
	var $elm$core$Task$onSelfMsg = F3((_v0, _v1, _v2) =>
		$elm$core$Task$succeed(_Utils_Tuple0),
	);
	var $elm$core$Task$cmdMap = F2((tagger, _v0) => {
		var task = _v0.a;
	...
	var $elm$json$Json$Encode$object = (pairs) =>
		_Json_wrap(
			A3(
				$elm$core$List$foldl,
				F2((_v0, obj) => {
	...
	var $author$project$Main$encodeRustCommand = (command) => {
		switch (command.$) {
			case "Init":
			...
			case "MarkTask":
				...
				return $elm$json$Json$Encode$object(
					_List_fromArray([
						...
						_Utils_Tuple2("done", $elm$json$Json$Encode$bool(done)),
					]),
				);
			default:
				return $elm$json$Json$Encode$object(
	...
	var $author$project$Main$toRust = _Platform_outgoingPort(
		"toRust",
		$elm$core$Basics$identity,
	);
	var $author$project$Main$init = (_v0) =>
		_Utils_Tuple2(
	...
	var $author$project$Main$Log = (a) => ({ $: "Log", a: a });
	var $author$project$Main$SendToRust = (a) => ({ $: "SendToRust", a: a });
	var $author$project$Main$UpdateTasks = (a) => ({ $: "UpdateTasks", a: a });
	var $elm$json$Json$Decode$decodeValue = _Json_run;
	var $elm$json$Json$Decode$list = _Json_decodeList;
	var $author$project$Main$Task = F2((name, done) => ({
		done: done,
		name: name,
	}));
	...
	var $author$project$Main$taskListDecoder = $elm$json$Json$Decode$list(
		$author$project$Main$taskDecoder,
	);
	var $author$project$Main$decodeValue = (x) => {
		var result = A2(
			$elm$json$Json$Decode$decodeValue,
			$author$project$Main$taskListDecoder,
			x,
		);
		if (result.$ === "Ok") {
			var tasks = result.a;
	...
	var $author$project$Main$fromRust = _Platform_incomingPort(
		"fromRust",
		$elm$json$Json$Decode$value,
	);
	var $author$project$Main$subscriptions = (model) =>
		$author$project$Main$fromRust($author$project$Main$decodeValue);
	var $elm$core$Platform$Cmd$batch = _Platform_batch;
	var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
	var $author$project$Main$update = F2((msg, model) => {
		switch (msg.$) {
			case "UpdateTasks":
				var tasks = msg.a;
				return _Utils_Tuple2(
					_Utils_update(model, { tasks: tasks }),
			...
			default:
				var field = msg.a;
				return _Utils_Tuple2(
					_Utils_update(model, { field: field }),
					$elm$core$Platform$Cmd$none,
				);
		}
	});
	var $author$project$Main$AddTask = (a) => ({ $: "AddTask", a: a });
	var $author$project$Main$ClearDoneTasks = { $: "ClearDoneTasks" };
	var $author$project$Main$UpdateField = (a) => ({ $: "UpdateField", a: a });
	var $elm$html$Html$Attributes$boolProperty = F2((key, bool) =>
		A2(_VirtualDom_property, key, $elm$json$Json$Encode$bool(bool)),
	);
	var $elm$html$Html$Attributes$autofocus =
		$elm$html$Html$Attributes$boolProperty("autofocus");
	...
	var $elm$virtual_dom$VirtualDom$Normal = (a) => ({ $: "Normal", a: a });
	var $elm$virtual_dom$VirtualDom$on = _VirtualDom_on;
	var $elm$html$Html$Events$on = F2((event, decoder) =>
		A2(
	...
	var $elm$html$Html$Events$onClick = (msg) =>
		A2(
			$elm$html$Html$Events$on,
			"click",
			$elm$json$Json$Decode$succeed(msg),
		);
	var $elm$html$Html$Events$alwaysStop = (x) => _Utils_Tuple2(x, true);
	var $elm$virtual_dom$VirtualDom$MayStopPropagation = (a) => ({
		$: "MayStopPropagation",
		a: a,
	});
	var $elm$html$Html$Events$stopPropagationOn = F2((event, decoder) =>
		A2(
			$elm$virtual_dom$VirtualDom$on,
			event,
	...
	var $elm$json$Json$Decode$at = F2((fields, decoder) =>
		A3($elm$core$List$foldr, $elm$json$Json$Decode$field, decoder, fields),
	);
	...
	var $elm$html$Html$Events$onInput = (tagger) =>
		A2(
	...
	var $elm$html$Html$Events$alwaysPreventDefault = (msg) =>
		_Utils_Tuple2(msg, true);
	var $elm$virtual_dom$VirtualDom$MayPreventDefault = (a) => ({
		$: "MayPreventDefault",
		a: a,
	});
	var $elm$html$Html$Events$preventDefaultOn = F2((event, decoder) =>
		A2(
			$elm$virtual_dom$VirtualDom$on,
			event,
	...
	var $elm$html$Html$Events$onSubmit = (msg) =>
		A2(
	...
	var $author$project$Main$MarkTask = (a) => ({ $: "MarkTask", a: a });
	var $elm$core$List$filter = F2((isGood, list) =>
		A3(
			$elm$core$List$foldr,
			F2((x, xs) => (isGood(x) ? A2($elm$core$List$cons, x, xs) : xs)),
			_List_Nil,
			list,
		),
	);
	var $elm$core$Tuple$second = (_v0) => {
		var y = _v0.b;
		return y;
	};
	var $elm$html$Html$Attributes$classList = (classes) =>
	...
	var $author$project$Main$viewTask = F2((index, task) =>
		A2(
			$elm$html$Html$div,
			_List_fromArray([
			...
			_List_fromArray([$elm$html$Html$text(task.name)]),
		),
	);
					...
					_List_fromArray([
						A2(
							...
							_List_fromArray([
								...
								$elm$html$Html$Events$onClick(
									$author$project$Main$SendToRust(
										$author$project$Main$ClearDoneTasks,
									),
								),
							]),
							_List_fromArray([
